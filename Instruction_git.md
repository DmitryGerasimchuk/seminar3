...
Запуск команды, которая позволяет сравнивать версии
зеленым и плюсом отображается новое в верссии, красным и минусом - удаленое в версии:

    git diff

Команда, чтобы работать с любой версией коммита ("машина времени"):

    git chechout 

Если прописать в команде имя коммита (полное или сокращенное - 4 символа), то можно вернуться к версии этого коммита и работать с ним (при этом на экране будет отобращаться символы данного периода). При этом более старые коммиты перестают отображаться, если запускать программу **git log --oneline** или **git log**, но эти версии не удалены (!): к ним всегда можно вернуться.

возвращение к ветке master (head - master):
